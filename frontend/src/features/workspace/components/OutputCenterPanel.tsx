  ...
  return (
    <div className="tab-stack" aria-labelledby="output-center-title">
      <Alert
        type="warning"
        showIcon
        message="Experimental downloads — not the Official Sponsor workbook"
        description="These files are generated by legacy website logic. Use Official Forecast > Output Center for the approved, hash-verified seven-sheet workbook."
      />
      <Card
        className="content-card"
        title={<span id="output-center-title">Experimental Output Center</span>}
        extra={(
          <Space wrap>
            <Button
              type="primary"
              ...
              onClick={onPrepare}
            >
              {data ? "Refresh experimental run" : "Prepare experimental run"}
            </Button>
            <Button
            ...
            >
              Website-generated Excel (Not Official)
            </Button>
            <Tooltip
              title={pdfReady ? "" : "PDF requires the website reportlab server dependency."}
            >
              <span>
                <Button
                  icon={<DownloadOutlined />}
      ...
      >
        <Paragraph className="workspace-copy" style={{ marginBottom: 0 }}>
          Prepare one website-owned experimental run for the current filters and settings. This
          output is retained for analysis and is not an approved Forecast API artifact.
        </Paragraph>
        <div aria-live="polite" role="status" style={{ marginTop: 12 }}>
          {loading ? "Preparing experimental output run." : data
            ? `Run ready: ${data.metadata.runId}`
            : "No experimental output run is ready. Downloads are disabled."}
        </div>
      </Card>
      ...
      {!data ? (
        <Alert
          type="info"
          showIcon
          message="Prepare an experimental output run to enable non-official preview and downloads."
        />
      ) : (
        <>
          ...
          <Card className="content-card" title="Experimental Summary preview">
            <Table
              rowKey="month"
              pagination={false}
          ...
          <Card className="content-card" title="Website reconciliation and Top 10 PLC">
